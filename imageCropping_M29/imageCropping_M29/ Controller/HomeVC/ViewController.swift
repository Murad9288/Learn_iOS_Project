...
class ViewController: UIViewController,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIScrollViewDelegate,UIGestureRecognizerDelegate
{
    ...
    @IBOutlet weak var ManiImageView: UIImageView!
...
//}
    ...
    {
        let croppedCGImage = ManiImageView.image?.cgImage?.cropping(to: cropArea)
        let croppedImage = UIImage(cgImage: croppedCGImage!)
        let storyboard = UIStoryboard(name: "Sub_Storyboard", bundle: nil)
        let controller = storyboard.instantiateViewController(withIdentifier: "SubViewController") as! SubViewController
        self.present(controller, animated: true, completion: nil)
        controller.imageView.image = croppedImage
    }
    
    var cropArea:CGRect
        {
        get{
            let factor = ManiImageView.image!.size.width/view.frame.width
            let scale = 1/scroll.zoomScale
            let imageFrame = ManiImageView.imageFrame()
    ...
    private func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any])
    {
        ...
        ManiImageView.contentMode = .scaleAspectFit
        ManiImageView.image = chosenImage.resizeImage(image: chosenImage, targetSize: CGSize(width: 3024 , height: 4032))
